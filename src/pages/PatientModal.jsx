...
//   let cancelled = false;
//   const API_BASE = import.meta.env.VITE_BACKEND_API || "api.twentytwohealth.com/rpm-be";
...
//     if (!fetched) {
//       console.warn("PatientModal: failed to fetch patient details for id", id);
//       return;
//     }
...
//       try { sessionStorage.setItem(`patientProfile_byId_${id}`, JSON.stringify(fetched)); } catch {}
//     } catch {}
...
//             patient: fetched,
//             patientName: fetched.name || fetched.username || selectedPatient.name,
//           },
//         })
//       );
//       console.log("PatientModal: dispatched openPatientFromBreadcrumb with fetched patient");
//     } catch (err) {
...
//       const headers =
//         deviceType === "bp"
//           ? ["Date & Time", "Blood Pressure", "Pulse", "Status", "Device"]
//           : ["Date & Time", "SpO₂", "Pulse", "PI", "Status", "Device"];
...
//         doc.rect(20 + index * 35, startY, 35, 8, "F");
//         doc.text(header, 22 + index * 35, startY + 6);
//       });
...
//                 record.bpStatus ? `${record.bpStatus} (Warning)` : "Normal",
...
//                     : "N/A",
//                   recordData.pi || "N/A",
//                   spo2Status.charAt(0).toUpperCase() + spo2Status.slice(1),
//                   getDeviceName(record),
//                 ];
//               })();
...
//           doc.rect(20, startY - 2, 175, 8, "F");
//         }

//         rowData.forEach((cell, cellIndex) => {
//           doc.text(String(cell), 22 + cellIndex * 35, startY + 6);
//         });
...
//         doc.text("Generated by RPM System", 195, 290, { align: "right" });
//       }
...
//                 status: "normal",
//                 timestamp:
//                   realTimeData?.heartRateTimestamp || new Date().toISOString(),
//               },
//               bloodPressure: {
...
//                 timestamp: realTimeData?.latestBP?.created_at,
//               },
//             };
//           case "spo2":
//             return {
...
//         loadHistoricalData(days, 1, patientData.patient.id);
//       } else {
//         if (fromDate && toDate) {
//           const from = new Date(fromDate);
//           const to = new Date(toDate);
//           const diffMs = Math.max(0, to.getTime() - from.getTime());
//           const days = Math.floor(diffMs / (1000 * 60 * 60 * 24)) + 1;
//           loadHistoricalData(days || daysFilter, 1, patientData.patient.id);
//         } else {
//           loadHistoricalData(daysFilter, 1, patientData.patient.id);
//         }
//       }
//     };
...
//               <button
//                 onClick={handleQuery}
//                 className="px-4 py-2 bg-[#103c63] text-white rounded text-sm  transition-colors flex items-center space-x-2"
//               >
//                 <RefreshCw size={16} />
//                 <span>Query</span>
//               </button>
...
//       try { window.__vital_patient_name = selectedPatient.name; } catch {}
//     }
//   } catch (err) {
...
//   const patientId =
//     selectedPatient?.id || selectedPatient?.patient_id || selectedPatient?._id;
//   if (!patientId) {
...
//   if (origin === "patient" || origin === "/patients" || origin === "patients") {
...
//   try {
//     const slugKey = encodeURIComponent(String(currentSlug).trim().toLowerCase().replace(/\s+/g,"-"));
//     sessionStorage.setItem(`patientId_bySlug_${slugKey}`, String(patientId));
//     if (selectedPatient?.name) sessionStorage.setItem(`patientName_bySlug_${slugKey}`, String(selectedPatient.name));
//     // cache profile if available
//     try { sessionStorage.setItem(`patientProfile_byId_${patientId}`, JSON.stringify(selectedPatient)); } catch {}
//   } catch {}
//   routerNavigate(`/dashboard/${encodeURIComponent(String(currentSlug).trim().toLowerCase().replace(/\s+/g,"-"))}/vital-signs/${patientId}`, {
//     state: routeState,
//   });
// };
...
//                           {selectedPatient.email || patientProfile?.email || "No email"}
//                         </p>
//                       </div>
//                     </div>
...
//                           {selectedPatient.phoneNumber || patientProfile?.phoneNumber || "Not provided"}
//                         </p>
//                       </div>
//                     </div>
//                   </div>
...
//                       </p>
//                       <p className="text-sm font-semibold text-gray-900 dark:text-white mt-1">
//                         {(selectedPatient.created_at || patientProfile?.created_at)
//                           ? new Date(
//                               selectedPatient.created_at || patientProfile?.created_at
...
//                       </p>
//                       <p className="text-sm font-semibold text-gray-900 dark:text-white mt-1">
//                         {formatDate(selectedPatient.last_login || patientProfile?.last_login)}
//                       </p>
//                     </div>
//                   </div>
//                 </div>
...
//                 {selectedPatient.patient_id || patientProfile?.patient_id || patientProfile?.id || "N/A"}
//               </span>
//             </div>
