...
// import { Loader, Laptop, Link as LinkIcon, ChevronDown, ChevronUp } from "lucide-react";
...
//       setLoading(true);
//       setError(null);
//       const response = await fetch(`${API_BASE}/api/doctor/assigned?page=1&limit=50`, {
//         method: "GET",
...
//       ...prev,
//       [userId]: { ...(prev[userId] || {}), open: willBeOpen, loading: willBeOpen },
//     }));
...
//     try {
//       const res = await fetch(`${API_BASE}/api/dev-data/devices-used/${userId}`, {
//         method: "GET",
//         headers: { "Content-Type": "application/json" },
//         credentials: "include",
//       });
...
//       // Fallback: dedupe from BP history if endpoint not available
//       if (!devices.length) {
//         try {
//           const hist = await fetch(`${API_BASE}/api/dev-data/getBPHistorical/${userId}?days=365&page=1`, { credentials: "include" });
//           if (hist.ok) {
//             const hjson = await hist.json();
//             const unique = new Map();
//             (hjson?.data || []).forEach((r) => {
//               const devId = r.dev_id || (typeof r.data === "string" ? (JSON.parse(r.data).deviceInfo?.name || r.dev_id) : r.data?.deviceInfo?.name) || r.dev_id;
//               if (!devId) return;
//               if (!unique.has(devId)) unique.set(devId, { id: devId, name: r.data?.deviceInfo?.name || devId, type: r.dev_type || "bp" });
//             });
//             devices = Array.from(unique.values());
//           }
//         } catch {}
//       }
...
//       }));
//     } catch (e) {
//       setDevicesByUserId((prev) => ({ ...prev, [userId]: { open: true, loading: false, devices: [] } }));
//     }
//   };

//   return (
//     <div className="space-y-6">
//       <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Device Management</h2>
...
//           <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Users & Devices</h3>
//         </div>
//         <div className="p-6">
//           {loading ? (
...
//                           <span className="text-white font-medium text-sm">{(p.name || p.username || "U").charAt(0).toUpperCase()}</span>
//                         </div>
//                         <div className="ml-4">
//                           <div className="text-sm font-medium text-gray-900 dark:text-white">{p.name || p.username}</div>
...
//                           <div className="text-sm text-gray-600 dark:text-gray-400">No devices found for this user.</div>
//                         ) : (
//                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
//                             {entry.devices.map((d) => (
//                               <div key={String(d.id || d.devId || d.name)} className="flex items-center space-x-3 p-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-800/50">
//                                 <div className="h-8 w-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
//                                   <Laptop className="w-4 h-4 text-gray-700 dark:text-gray-200" />
//                                 </div>
//                                 <div className="flex-1 min-w-0">
//                                   <div className="text-sm font-medium text-gray-900 dark:text-white truncate">{d.name || d.dev_type || `Device ${d.devId || d.id}`}</div>
//                                   <div className="text-xs text-gray-500 dark:text-gray-400 truncate">ID: {d.devId || d.id}</div>
//                                 </div>
//                               </div>
//                             ))}
//                           </div>
